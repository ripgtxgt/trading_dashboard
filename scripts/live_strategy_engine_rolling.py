...
class LiveStrategyEngineRolling:
    ...
    def generate_signal(self, ohlcv_data):
        ...
        if long_cond1 and long_cond2 and long_cond3:
            signal_type = 'long'
            reason = "All long conditions met, open long position"
        elif short_cond1 and short_cond2 and short_cond3:
            signal_type = 'short'
            reason = "All short conditions met, open short position"
        else:
            # 分析为什么没有信号
            if not long_cond1 and not short_cond1:
                reason = "Waiting for MA cross: MA5 and MA20 too close"
            elif long_cond1:
                if not long_cond2:
                    reason = "Waiting for price breakout: price needs to break MA5"
                elif not long_cond3:
                    reason = "Waiting for trend confirmation: MA5 needs to keep rising"
            elif short_cond1:
                if not short_cond2:
                    reason = "Waiting for price breakout: price needs to fall below MA5"
                elif not short_cond3:
                    reason = "Waiting for trend confirmation: MA5 needs to keep falling"
