...
class TradingStrategy:
    ...
    def _init_db_state(self):
        ...
        try:
            ...
            logger.info("DatabaseInitializeSuccess")
    ...
    def open_position(self, signal: str, price: float):
        ...
        if self.position is not None:
            logger.warning("Position, CannotOpen position")
            return
        ...
        logger.info(f"Open positionSuccess: {signal} @ {price}, Amount: {quantity:.4f}")
    ...
    def close_position(self, price: float, reason: str = "止盈/止损"):
        ...
        logger.info(f"Close positionSuccess: {side} @ {price}, : {pnl:.2f} ({pnl_percent:.2f}%)")
    ...
    def check_stop_loss(self, current_price: float) -> bool:
        ...
        if side == 'long':
            ...
            if current_price <= stop_price:
                logger.warning(f"Stop loss: {current_price} <= {stop_price}")
    ...
    def check_take_profit(self, current_price: float) -> bool:
        ...
        if side == 'long':
            ...
            if current_price >= take_price:
                logger.info(f"Take profit: {current_price} >= {take_price}")
    ...
    def run_cycle(self, cycle_num: int):
        """运行一个交易周期"""
        logger.info(f"\n{'='*50}")
        logger.info(f"Period #{cycle_num} Start")
        ...
        if signal and self.position is None:
            ...
            logger.info(f"Position: {self.position['side']} @ {self.position['entry_price']:.2f}")
    ...
    def run(self, max_cycles: Optional[int] = None):
        ...
        """
        logger.info("="*60)
        logger.info("10U - Start")
        logger.info("="*60)
        logger.info(f"Trade: {self.symbol}")
        logger.info(f"Capital: {self.initial_balance} USDT")
        logger.info(f": {self.leverage}x")
        logger.info(f"MA: {self.short_ma_period}/{self.long_ma_period}")
        ...
        try:
            while True:
                ...
                if max_cycles and cycle_num >= max_cycles:
                    logger.info(f"ReachPeriod {max_cycles}, StopRunning")
                    break
                
                # 等待下一个周期(1小时)
                # 实际使用时根据timeframe调整
                sleep_seconds = 3600  # 1小时
                logger.info(f"Waiting {sleep_seconds} ...")
...
def main():
    ...
    if missing_vars:
        logger.error(f": {', '.join(missing_vars)}")
        logger.error("Please: ")
        logger.error("  DATABASE_URL - MySQLDatabaseConnect")
